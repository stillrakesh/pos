...
const formatMenuItem = (item) => {
  ...
  let modifier_groups = [];
  let add_ons = [];
  try { modifier_groups = JSON.parse(item.modifier_groups || '[]'); } catch(e){}
  try { add_ons = JSON.parse(item.add_ons || '[]'); } catch(e){}
...
router.get('/', (req, res) => {
  try {
    ...
    // Convert available int → boolean and parse modifiers
    const cleaned = items.map(formatMenuItem);
...
router.get('/:id', (req, res) => {
  try {
    ...
    let item = statements.getMenuById({ id });
    if (!item) {
...
router.post('/', (req, res) => {
  try {
    let { items, name, category, price, available, inStock, type, short_code, shortCode, modifier_groups, add_ons } = req.body;
    
    // Support aliases
    if (short_code === undefined && shortCode !== undefined) short_code = shortCode;
      ...
      items.forEach(mi => {
        ...
        if (existing) {
          statements.updateMenuItem({
            ...
            type: mi.type || 'Veg',
            short_code: mi.short_code || mi.shortCode || existing.short_code,
          ...
          statements.insertMenuItem({
            ...
            available: itemAvailable !== false,
            type: mi.type || 'Veg',
            short_code: mi.short_code || mi.shortCode || null,
            modifier_groups: mi.modifier_groups,
            add_ons: mi.add_ons
          });
        }
    ...
    const result = statements.insertMenuItem({
      ...
      available,
      type: type || 'Veg',
      short_code: short_code ? String(short_code).trim() : null,
      modifier_groups,
      add_ons
    });

    let item = statements.getMenuById({ id: result.lastInsertRowid });
    item = formatMenuItem(item);
...
router.put('/:id', (req, res) => {
  try {
    ...
    let { name, category, price, available, inStock, short_code, shortCode, type, modifier_groups, add_ons } = req.body;
    
    // Support POS frontend alias
    if (available === undefined && inStock !== undefined) {
      available = inStock;
    }
    if (short_code === undefined && shortCode !== undefined) {
    ...
    statements.updateMenuItem({
      ...
      available,
      type,
      short_code: short_code !== undefined ? (short_code ? String(short_code).trim() : null) : undefined,
      modifier_groups,
      add_ons
    });

    let updated = statements.getMenuById({ id });
    updated = formatMenuItem(updated);
